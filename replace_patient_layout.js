...
const replacement = `<div className="hidden lg:flex items-center max-w-md w-full ml-4 relative">
                <FiMenu className="w-4 h-4 text-slate-400 absolute left-3 hidden" />
                <input 
                  type="text" 
...
if (regex.test(content)) {
  ...
  console.log("Successfully replaced the patient header layout left part!");
} else {
  console.error("Regex did not match anything in patient layout.");
}
